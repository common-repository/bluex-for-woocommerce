...
abstract class WC_Correios_Shipping extends WC_Shipping_Method {
	...
	public function __construct( $instance_id = 0 ) {
		...
		$this->method_description = sprintf( __( '%s is a shipping method from Blue Express.', 'woocommerce-correios' ), $this->method_title );
		$this->supports           = array(
			'shipping-zones',
			'instance-settings',
		);
...
//exit;
	...
	public function calculate_shipping( $package = array() ) {
		if ( '' === $package['destination']['postcode'] || 'CL' !== $package['destination']['country'] ) {
//			return;
		}
		...
		if ($shipping->isShipmentFree == false && 0 === intval( $cost )) {
			return;
		}
		...
		$meta_delivery = array(
			'_delivery_forecast' => $shipping->PrazoEntrega
		);
